...
v_init=-L/40
v_fin=L/40
v,v_mid = v_barrera(h,v_init,v_fin)
#v = v_caixa()
#v = v_harmonic()
#d=1.; h=2.; v = v_dent_de_serra(d,h)

# Manual without animation
for value in range(len(v_values)):
    v = np.zeros_like(x)
    v,v_mid = v_barrera(v_values[value],v_init,v_fin)
    ## Build WP    
    ...
    diff = np.zeros(1+int(steps))
    t_pas = np.zeros(1+int(steps))

    norm[0]=n(psi2)
    refl[0]= r(psi2,v_mid)
    trans[0]= t(psi2,v_mid)
    diff[0]=norm[0]-refl[0]-trans[0]
    t_pas[0]=0.

    for i in range(int(steps)):
        j = i + 1
        f90.euler_pbc(dt=dt,dx=dx,m=m,h_barra=h_barra,v=v,psi_r=psi_r,psi_i=psi_i,steps_print=steps_print)
        ...
        diff_val = norm_val-refl_val-trans_val
...
def animate(i):
    ...
    diff_val = norm_val-refl_val-trans_val
    diff_text.set_text(diff_template % (diff_val))
    trans[i]=trans_val
    refl[i]=refl_val
    t_pas[i]= t_pas_val
    return quadrat_line,t_pas_text, norm_text, refl_text, trans_text, diff_text#, real_line, imagine_line
...
diff_text = ax.text(0.05, 0.65, '', transform=ax.transAxes)
plt.tight_layout()
...
plt.title('Reflexion & Transmission of a WP vs Barrier height',fontsize=16)
plt.plot(v_values,t_v,label='WP Transmission')
plt.plot(v_values,r_v,label='WP Reflexion')
plt.xlabel('Barrier Height (a.u.)',fontsize=14)
plt.legend(loc = 'best')
plt.xticks([v_values[0],v_values[int(v_points/3)],v_values[int(2*v_points/3)],v_values[-1]],['0','E/2','E','3E/2'])
plt.savefig('barrier_height.png',format='png',dpi=200)
plt.savefig('barrier_height.eps',format='eps')
#plt.show()
