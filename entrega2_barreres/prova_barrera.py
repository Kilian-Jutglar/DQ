...
v_init=-L/40
v_fin=L/40
v_values=np.linspace(0,L/20,50)
...
def animate(i):
    ...
    suma_val=refl_val+trans_val
    suma_text.set_text(suma_template % (suma_val))
    trans[i]=trans_val
    refl[i]=refl_val
    t_pas[i]= t_pas_val
    return quadrat_line,t_pas_text, norm_text, refl_text, trans_text,suma_text, real_line, imagine_line
...
suma_text = ax.text(0.05, 0.2, '', transform=ax.transAxes)
plt.tight_layout()
