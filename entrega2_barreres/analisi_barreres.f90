...
implicit none
logical                 ::      pbc
integer                 ::      i, step, step_print, barrier
...
real(8)                 ::      start, finish
real(8)                 ::      T, R,Told=0.d0, Rold=1.d0
real(8)                 ::      init, fin, barrera, max_barrera
real(8)                 ::      time
...
dt_crit=(2*m*hBarra*dx**2)/(hBarra**2 + 2.d0*m*dx**2*max_barrera)
...
! Open output file
open(unit=un_out,file='RT.log',status='new')
open(unit=55,file='der_RT.log',status='new')

Rold = dx*(sum(psiR(1:init_point)**2+ psiI(1:init_point)**2))
Told = dx*(sum(psiR(fin_point:xpoints)**2+ psiI(fin_point:xpoints)**2))

!Starting loop propagation
!do barrier=30,31
        barrera = (float(barrier)-1.d0)*max_barrera/float(49)
        v=v_barrera()
        time = 0.d0
        R = dx*(sum(psiR(1:init_point)**2+ psiI(1:init_point)**2))
        print*,'R',R
        Rold = R
        T = dx*(sum(psiR(fin_point:xpoints)**2+ psiI(fin_point:xpoints)**2))
        print*,'T',T
        Told=T
        !do while (Rold>R .and. Told<T)
        do step=1,Nsteps
                time = time + dt
                call euler()
                Rold = R
                ...
                R = dx*(sum(psiR(1:init_point)**2+ psiI(1:init_point)**2))
                T = dx*(sum(psiR(fin_point:xpoints)**2+ psiI(fin_point:xpoints)**2))
                write(un_out,*) time, R, T 
                write(55,*) time, Rold - R, Told - T 
        enddo
        !print*,'Barrier',barrier,'done'
!enddo
! Check final Norm
